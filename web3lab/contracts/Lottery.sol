...
//   1. A manager address
    ...
    {
        require(msg.value==0.0001 ether, "Need to pay 0.0001 ether to participate in the lottery");
    ...
    function randomNumberGenerator () private view returns (uint)
    {
      ...
      return uint(keccak256(abi.encodePacked(block.difficulty,timestamp, participants.length)));
    ...
    function luckyWinner () public 
    {
           ...
           require (participants.length >=3, "At least 3 person needed to start a lottery");
           uint rand = randomNumberGenerator();
          
           uint index = rand %getParticipantLength();
